...
def main():
    ...
    fig.suptitle('TSkin Sensor Data with gravity compensation')
    
    # Configure subplots
    ax_angle, ax_acc, ax_lin_acc, ax_gyro, ax_touch = axs
    ...
    ax_acc.set_ylabel('Acceleration [raw] (m/s²)')
    line_acc_x, = ax_acc.plot([], [], label='X')
    line_acc_y, = ax_acc.plot([], [], label='Y')
    line_acc_z, = ax_acc.plot([], [], label='Z')
    ax_acc.legend()

    ax_lin_acc.set_ylabel('Linear Acc (m/s²)')
    line_lin_x, = ax_lin_acc.plot([], [], label='X')
    line_lin_y, = ax_lin_acc.plot([], [], label='Y')
    line_lin_z, = ax_lin_acc.plot([], [], label='Z')
    ax_lin_acc.legend()
    ...
    buffers = {
        ...
        'lin_x': deque(maxlen=max_length),
        'lin_y': deque(maxlen=max_length),
        'lin_z': deque(maxlen=max_length)
    }
    ...
    while True:
        ...
        if a and acc:
            try:
                ...
                roll = math.radians(a.roll) if a.roll else 0.0
                pitch = math.radians(a.pitch) if a.pitch else 0.0

                # Calculate gravity components
                g_x = G_SCALE * GRAVITY * math.sin(pitch)
                g_y = -G_SCALE * GRAVITY * math.sin(roll) * math.cos(pitch)
                g_z = G_SCALE * GRAVITY * math.cos(roll) * math.cos(pitch)

                # Compensate for gravity
                lin_x = acc.x - g_x if acc.x else 0.0
                lin_y = acc.y - g_y if acc.y else 0.0
                lin_z = acc.z - g_z if acc.z else 0.0
        ...
        buffers['lin_x'].append(lin_x)
        buffers['lin_y'].append(lin_y)
        buffers['lin_z'].append(lin_z)
        ...
        line_lin_x.set_data(x_data, buffers['lin_x'])
        line_lin_y.set_data(x_data, buffers['lin_y'])
        line_lin_z.set_data(x_data, buffers['lin_z'])
        ax_lin_acc.relim()
        ax_lin_acc.autoscale_view()
        ...
        writer.writerow([timestamp, roll_log, pitch_log, yaw_log, touch_gesture_log,
                         acc_x_log, acc_y_log, acc_z_log,lin_x,lin_y,lin_z, gyro_x_log, gyro_y_log, gyro_z_log])
